...
TBL lock_nsymbol_t{
    ...
    time_point_sec  lock_ended_at;

    lock_nsymbol_t() {}
    lock_nsymbol_t(const nsymbol& s): symbol(s) {};

    uint64_t primary_key()const { return symbol.raw(); }

    EOSLIB_SERIALIZE(lock_nsymbol_t, (symbol)(lock_started_at)(lock_ended_at) )

    typedef eosio::multi_index< "locknsymbols"_n, lock_nsymbol_t > idx_t;
};
