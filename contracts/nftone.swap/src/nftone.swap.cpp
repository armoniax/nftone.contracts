...
void blindbox::_rand_nft( pool_t pool , const name& owner , deal_trace_t trace){
    ...
    nft.erase( itr );
    add_amount( pool.id, owner,1);
    _on_deal_trace(trace);

    pool_t::tbl_t pt( get_self() ,get_self().value );
    auto pool_itr           = pt.find( pool.id );
    pt.modify( pool_itr, same_payer, [&]( auto& row ) {
        row.not_exchange_nft_amount     -= amount;
        row.exchange_nft_amount         += amount;
        row.updated_at                  = current_time_point();
...
void blindbox::add_amount( const uint64_t& pool_id, const name& owner, const uint64_t& amount){
