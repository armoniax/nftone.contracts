...
void rndnft_swap::editplantime(const name& owner, const uint64_t& pool_id, const time_point_sec& opended_at, const time_point_sec& closed_at){
    pool_t::tbl_t pool_tbl( get_self() ,get_self().value );
    auto pool_itr = pool_tbl.find(pool_id);
    CHECKC( pool_itr != pool_tbl.end(),  err::RECORD_NOT_FOUND,"pool not found: " + to_string(pool_id) )
...
void rndnft_swap::ontranstoken( const name& from, const name& to, const asset& quantity, const string& memo ){
    _on_open_transfer( from, to, quantity,memo);
}

void rndnft_swap::ontransnft( const name& from, const name& to, const vector<nasset>& assets, const string& memo){
...
void rndnft_swap::dealtrace(const deal_trace_t& trace){
...
void rndnft_swap::_buy_one_nft( pool_t& pool , const name& owner , deal_trace_t trace, const uint64_t& amount){
...
void rndnft_swap::_add_nfts( pool_t& p, const vector<nasset>& quants ){
...
void rndnft_swap::_on_deal_trace(const deal_trace_t& deal_trace)
{
