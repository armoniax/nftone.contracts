...
class rndnft_mart_base_tester : public tester {
public:

   rndnft_mart_base_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol) }, false, false );
      create_accounts( { N(rndnft.mart) } , false, true);
      produce_blocks( 2 );

      set_code( N(rndnft.mart), contracts::rndnft_mart_wasm() );

      set_abi( N(rndnft.mart), contracts::rndnft_mart_abi().data() );

      produce_blocks();

      const auto& accnt = control->db().get<account_object,by_name>( N(rndnft.mart) );
      abi_def abi;
   ...
   {
      vector<char> data = get_row_by_account( N(rndnft.mart), N(rndnft.mart), N(auditors), auditor );
   ...
   {
      vector<char> data = get_row_by_account( N(rndnft.mart), N(rndnft.mart), table_name, pk);
   ...
   //    vector<char> data = get_row_by_account( N(rndnft.mart), acc, N(accounts), account_name(symbol_code) );
   ...
   action_result action_init( uint8_t score_limit ) {
      return push_action( N(rndnft.mart), N(init), mvo()
           ( "score_limit", score_limit)
      );
   }

   action_result action_setscore( name audit_type, int8_t score ) {
      return push_action(  N(rndnft.mart), N(setscore), mvo()
           ( "audit_type", audit_type)
           ( "score", score)
      );
   }
   action_result action_setauditor( const name& account, const set<name>& actions  ) {
      return push_action(  N(rndnft.mart), N(setauditor), mvo()
           ( "account", account)
           ( "actions", actions)
      );
   }
   ...
   action_result action_bindaccount(  const name& admin, const name& account, const string& number_hash ) {
      return push_action(  N(rndnft.mart), N(bindaccount), mvo()
           ( "admin",         admin)
           ( "account",       account)
   ...
   action_result action_bindanswer( const name& admin, const name& account, map<uint8_t, string>& answers ) {
      return push_action(  N(rndnft.mart), N(bindanswer), mvo()
           ( "admin",      admin)
           ( "account",    account)
           ( "answers",    answers)
      );
   ...
   action_result action_createorder(   const name&          admin,
      ...
      return push_action(  N(rndnft.mart), N(createorder), mvo()
           ( "admin",            admin)
           ( "account",          account)
   ...
   action_result action_chkanswer( const name&  admin,
                        ...
                        const name&             account,
                        const int8_t&           score) {
      return push_action(  N(rndnft.mart), N(chkanswer), mvo()
           ( "admin",      admin)
           ( "order_id",   order_id)
           ( "account",    account)
           ( "score",      score)
   ...
   action_result action_chkdid( const name& admin,
                        ...
                        const name& account,
                        const bool& passed) {
      return push_action(  N(rndnft.mart), N(chkdid), mvo()
           ( "admin",      admin)
           ( "order_id",   order_id)
           ( "account",    account)
           ( "passed",     passed)
   ...
   action_result action_chkmanual( const name& admin,
      ...
      return push_action(  N(rndnft.mart), N(chkmanual), mvo()
           ( "admin",      admin)
           ( "order_id",   order_id)
           ( "account",    account)
           ( "passed",     passed)
   ...
   action_result action_delorder( const name& submitter, const uint64_t& order_id ) {
      return push_action(  N(rndnft.mart), N(delorder), mvo()
           ( "submitter",  submitter)
           ( "order_id",   order_id)
      );
   }
